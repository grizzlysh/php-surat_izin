<?php
    ...
    class Dept_model extends CI_Model
    {
        ...
        {
            return $this->db->get_where($this->_table, ["id_dept" => $id])->row();
        }

        public function cekID(){
            $akhir = $this->db->select('id_dept')->order_by('id_dept','desc')->limit(1)->get($this->_table)->row('id_dept');            
            $hasil="";
            // var_dump($akhir);
       ...
       //     $this->kd_surat = uniqid();
            $this->id_dept = $this->cekID();
            $this->nama_dept = $post["nama"];
